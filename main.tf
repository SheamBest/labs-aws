...
module "dynamo_db_courses" {
  source  = "./modules/dynamodb/eu-west-1"
  context = module.base_labels.context
  name    = "courses"
}
...
module "lambda" {
  source  = "./modules/lambda/eu-west-1"
  context = module.base_labels.context
  name    = "lambda"
...
module "Notified_Lambda" {
  source            = "./modules/Notified_Lambda/eu-west-1"
  context           = module.base_labels.context
...
module "budget" {
  source                     = "./modules/budget/eu-west-1"
  context                    = module.base_labels.context
  name                       = "budget"
